package com.ikeda.payment.controllers;

import com.ikeda.payment.configs.security.AuthenticationCurrentUserService;
import com.ikeda.payment.configs.security.UserDetailsImpl;
...
import com.ikeda.payment.specifications.SpecificationTemplate;
...
public class PaymentController {
    ...
    public ResponseEntity<Object> requestPayment(@PathVariable(value = "userId")UUID userId,
                                                 @RequestBody
                                                 @Valid PaymentRequestRecordDto paymentRequestRecordDto){
        ...
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(paymentService.requestPayment(paymentRequestRecordDto, userModel));
    }
    ...
    public ResponseEntity<Object> getOnePayments(@PathVariable(value = "userId") UUID userId,
                                                 @PathVariable(value = "paymentId") UUID paymentId){
        UserDetailsImpl userDetails = authenticationCurrentUserService.getCurrentUser();
        if (userDetails.getUserId().equals(userId) || userDetails.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_ADMIN"))){
            return ResponseEntity.status(HttpStatus.OK).body(paymentService.findPaymentByUser(userId, paymentId).get());
        } else {
            throw new AccessDeniedException("Forbidden");
        }
